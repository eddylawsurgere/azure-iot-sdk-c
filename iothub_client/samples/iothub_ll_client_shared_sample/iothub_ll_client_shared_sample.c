...
static const DEVICE_CREDENTIAL g_credentials[] = {
    { .id = "<device id 1>", .key = "<device key 1>" },
    { .id = "<device id 2>", .key = "<device key 2>" }
};

#define NUMBER_OF_DEVICES sizeofarray(g_credentials)
...
static IOTHUBMESSAGE_DISPOSITION_RESULT ReceiveMessageCallback(IOTHUB_MESSAGE_HANDLE message, void* userContextCallback)
{
    ...
    if (contentType == IOTHUBMESSAGE_BYTEARRAY)
    {
        ...
        {
            (void)printf("Received Message [%s; %lu]\r\n Message ID: %s\r\n Correlation ID: %s\r\n BINARY Data: <<<%.*s>>> & Size=%d\r\n", 
...
int main(void)
{
    ...
    {
        g_device_states[i].credential = &g_credentials[i];
    ...
    else
    {
        ...
        for (int i = 0; i < NUMBER_OF_DEVICES && setup_ok; i++)
        {
            ...
            {
                (void)printf("ERROR: iotHubClientHandle is NULL (%d)!\r\n", i);
                setup_ok = false;
            }
            else
            {            
                ...
                {
                    (void)printf("Failed setting log tracing (%d)!\r\n", i);
                    setup_ok = false;
                }
    #ifdef SET_TRUSTED_CERT_IN_SAMPLES
                ...
                {
                    (void)printf("Failed setting cloud-to-device message callback (%d)!\r\n", i);
                    setup_ok = false;
                }
            }
        }
        ...
        if (setup_ok)
        {
            ...
            do
            {
                ...
                if (messages_sent > MESSAGE_COUNT) g_continueRunning = false;
